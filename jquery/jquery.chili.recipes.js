...
jQuery.chili.recipes.php =
{
	...
	, php: {
		...
		, class_keyword: {
			  _match: /\b(private|public|protected|self|static|Class|const)\b/ 
			, _style: "color: #2FA41F; font-weight: bold;"
		}
		, keyword: {
			  _match: /\b(?:__CLASS__|__FILE__|__FUNCTION__|__LINE__|__METHOD__|abstract|and|array|as|break|case|catch|cfunction|clone|continue|declare|default|die|do|echo|else|elseif|empty|enddeclare|endfor|endforeach|endif|endswitch|endwhile|eval|exception|exit|extends|extends|final|for|foreach|function|global|if|implements|include|include_once|interface|isset|list|new|old_function|or|php_user_filter|print|require|require_once|return|switch|this|throw|try|unset|use|var|while|xor)\b/ 
			, _style: "color: navy; font-weight: bold;"
		}
		, variable: {
			  _match: /\$(\w+)/
			, _replace: '<span style="color: #D2C614; font-weight: bold;">$</span><span class="variable">$1</span>' 
			, _style: "color: #2D92B4;"
		}
		, heredoc: {
			  _match: /(\<\<\<\s*)(\w+)((?:(?!\2).*\n)+)(\2)\b/
...
jQuery.chili.recipes.js =
{
	...
	, _main: {
		...
		, brace: { 
			  _match: /[\{\}]/
			, _style: 'color: #282BA9; font-weight: bold;'
		}
		, parens: { 
			  _match: /[\(\)]/
			, _style: 'color: #282BA9; font-weight: bold;'
		...
		, property: { 
			  _match: /\b(document|window|undefined|arguments|NaN|Infinity)\b/
			, _style: 'color: #CCA71E; font-weight: bold;'
		}
		, 'function': { 
